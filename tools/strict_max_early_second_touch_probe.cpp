...
#if defined(PROBE_EARLY_TOUCH_ONLY)
  const int target_early_touch = 1200;
#else
#error "Define PROBE_EARLY_TOUCH_ONLY"
#endif
  int early_touch = 0;
  long long attempts = 0;
  while (attempts++ < 240000000LL && early_touch < target_early_touch) {
    // Early-second-touch source states live next to the retained-inner class:
    // the same rounded strict-max geometry, but the touched/outgoing TopX
    // values diverge at the first predecessor scanbeam.
    const IntPoint p(
    ...
    const bool early_touch_state =
        touched_position == 0 &&
        crossing_position == 1 &&
        touch_end_inside &&
        touch_end.y() < owner[2].y() &&
        other_third.y() < touch_end.y() &&
        top_x(touched, owner[2].y()) != top_x(outgoing, owner[2].y());
    if (!early_touch_state) continue;

    const Path expected = rebase_rightmost_min_successor(owner);
  ...
  std::cout << "generated early_touch=" << early_touch << "/"
            << target_early_touch << " bases\n";
  std::cout << "generated raw paths=" << early_touch * 18 << "/"
            << target_early_touch * 18 << " exact\n";
  return early_touch == target_early_touch ? 0 : 4;
}
