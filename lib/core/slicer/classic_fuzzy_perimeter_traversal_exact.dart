...
/// Exact-RNG no-painted-region classic `traverse_loops()` branch.
///
/// Sibling polygons are fuzzified in source loop order before nearest-neighbor
/// chaining. Children are fuzzified only when recursion visits their selected
/// parent, preserving both independent thread-local RNG states across the same
/// call order as source.
class SourceClassicFuzzyPerimeterTraversalExact2 {
  const SourceClassicFuzzyPerimeterTraversalExact2._();
  ...
  static List<ExtrusionEntity2> _traverse({
    ...
    for (final loop in loops) {
      final polygon = SourceFuzzySkinNoRegionExactApply2.applyPolygon(
    ...
    if (thinWalls.isNotEmpty) {
      final bounds = _ExactFuzzyBounds2();
