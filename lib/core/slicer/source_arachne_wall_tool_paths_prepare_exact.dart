...
/// bottom point-contact ties, and the represented partial-collinear triangle
/// joins, including decreasing-Y host-end and all non-horizontal staggered
/// non-fixup states, with exact `BuildResult()` starts/order for their asserted
/// contracts. Interacting holes, >2 interacting paths, remaining contact/fixup
/// states, mixed crossing/contact cases, wider-convex contact output state,
/// multi-reflex/nonlocal non-orthogonal cleanup and orthogonal hole/point-touch
/// ambiguity remain explicit compatibility seams. All post-offset cleanup
/// stages are the direct ports already hosted by
...
class SourceArachneWallToolPathsPrepareExact2 {
  ...
  ///   bounded contact helper; equal-bottom point contacts use their exact
  ///   reverse-source-order helper; non-horizontal staggered triangle joins use
  ///   the all-slope exact staggered helper; represented endpoint-aligned or
  ///   horizontal joins use the partial-collinear helper; the remaining
  ///   decreasing-Y host-end triangle state uses its exact helper; and exactly
  ///   two strict positive convex paths with only proper crossings use the exact
  ///   scanline-rounded convex union subset. Other interacting sets fall back
  ///   only at the final union;
