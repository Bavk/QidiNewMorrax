...
## Current executable checkpoint — 2026-09-18

- Flutter **3.47.2**, Dart **3.13.2**;
- validated code `81691379c94382e5218c3feb22ff182e9f36bd98` (clean branch checkpoint covering the first-scanbeam-divergent early-second-touch rounded strict-max state);
- workflow `35362020769` (#647), job `105655340480`, conclusion **success**;
- `flutter analyze` — **No issues found!**;
- `flutter test --reporter expanded` — **875/875 passing**.

The current suite retains every earlier represented Classic/Arachne/geometry fixture, #615's rounded-to-touch AEL-contained collapse, #639's two independently proved AEL-outside retained-output branches, and #647's first-scanbeam-divergent early-second-touch owner-only branch. The retained-wedge branch requires no inner-other crossing of the owner-predecessor scanbeam; the retained-inner branch requires the touched and outgoing owner bounds to remain integer-`TopX` tied through that first scanbeam, while #647 covers the corresponding divergence that processes the second same-coordinate touch early. Exact all-rotation raw paths and Arachne zero-offset routing are locked. Extra-`TopX` rounded neighbors, side, horizontal and mixed-collinear states remain fallback.
...
- #639 rounded-to-touch **AEL-outside retained-output strict-maximum branches**: pinned-source run `35284009243`; wedge job `105412173334` matched fixed **18/18** plus **1200/1200 bases = 21600/21600 exact full raw paths**, and retained-inner job `105412173551` independently matched fixed **18/18** plus **1200/1200 bases = 21600/21600 exact full raw paths**. Source trace `35283858278`, job `105411701842`, proves the retained-inner first-predecessor-scanbeam `TopX` tie boundary against the owner-only early-second-touch counterstate.
- #647 rounded-to-touch **AEL-outside early-second-touch owner-only branch**: direct pinned-source run `35361763642`, job `105654487004`, generated **1200/1200** independent first-scanbeam-divergent bases and matched **21600/21600 exact complete raw paths** across all 18 rotation/input-order variants. The earlier source-event trace `35283858278` / `105411701842` independently shows the divergence processing the second same-coordinate touch at `touch.y` and removing the inner endpoint.

The #575/#588/#582/#601/#608 strict-max predicates partition the represented non-horizontal ordinary-intersection ordering seam. #575 owns `otherThird.y < min(ownerPrevious.y, ownerNext.y)` with 1–4 proper crossings. #588 owns equality with at least one proper crossing. #582 plus #601 plus #608 own `otherThird.y > min(...)` across the proved single- and multi-crossing states, with source matrices spanning proper-count 1–4. The equal-Y and late separated-minimum audits establish narrow states where a valid raw contour may contain two **nonadjacent** global minimum-Y vertices; #601 independently proves that endpoint-Y boundary for multi-crossing and #608 independently proves it for single-crossing. #615 is orthogonal to that rebase partition and covers the rounded-to-touch AEL-contained `WindCnt == 1` collapse. #639 adds two independently proved AEL-outside `WindCnt == 2` retained-output branches: a no-extra-scanbeam wedge and a retained-inner state whose touched/outgoing bounds remain `TopX` tied through the first predecessor scanbeam. #647 adds the complementary first-scanbeam-divergent early-second-touch owner-only state. Extra-`TopX` rounded neighbors, horizontal strict-max touches, side-vertex touches and mixed collinear cases remain explicit fallback.
...
| modified Clipper1 represented offset/Execute/NonZero subsets | `source_clipper1_*` | direct pinned ELF/source oracles + #647 CI | `parity_verified` (exact fixture scopes) | remaining mixed states, wider/multi-point fixup, interacting holes, >2 paths and broader Execute cleanup |
...
1. Continue **mixed proper-crossing + point-touch/collinear two-positive paths** beyond the #569/#575/#582/#588/#601/#608/#615/#639/#647 proved classes: first the remaining rounded AEL-outside neighbors that emit additional scanbeam `TopX` vertices, then side-vertex touches, horizontal touch ordering and mixed collinear cases. Start from traced AEL / `AppendPolygon()` / `OutRec::Pts` state rather than assuming a geometric rebase rule.
