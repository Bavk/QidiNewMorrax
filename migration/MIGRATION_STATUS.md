...
- validated code `9f04863ea904552d8edd7034067630bd82f8be98` (clean branch checkpoint covering bounded AEL-outside retained-wedge and retained-inner rounded strict-max states);
- workflow `35284252825` (#639), job `105412942943`, conclusion **success**;
- `flutter analyze` — **No issues found!**;
- `flutter test --reporter expanded` — **874/874 passing**.

The current suite retains every earlier represented Classic/Arachne/geometry fixture, #615's rounded-to-touch AEL-contained collapse, and two independently proved AEL-outside retained-output branches. The retained-wedge branch requires no inner-other crossing of the owner-predecessor scanbeam; the retained-inner branch requires the touched and outgoing owner bounds to remain integer-`TopX` tied through that first scanbeam. Exact all-rotation raw paths and Arachne zero-offset routing are locked. Early-second-touch and extra-`TopX` rounded neighbors, side, horizontal and mixed-collinear states remain fallback.
...
| modified Clipper1 represented offset/Execute/NonZero subsets | `source_clipper1_*` | direct pinned ELF/source oracles + #639 CI | `parity_verified` (exact fixture scopes) | remaining mixed states, wider/multi-point fixup, interacting holes, >2 paths and broader Execute cleanup |
...
1. Continue **mixed proper-crossing + point-touch/collinear two-positive paths** beyond the #569/#575/#582/#588/#601/#608/#615/#639 proved classes: first the remaining rounded AEL-outside neighbors that process an early second touch or emit additional scanbeam `TopX` vertices, then side-vertex touches, horizontal touch ordering and mixed collinear cases. Start from traced AEL / `AppendPolygon()` / `OutRec::Pts` state rather than assuming a geometric rebase rule.
